...
# Pack ID: ITRGA-V2-0047-VERIFY-PACK-V2
...
#   root, ONLY after ITRGA_V2_0047_APPLY_PACK_V2.ps1 has PASSed:
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_VERIFY_PACK_V2.ps1"
#
...
#   3. Send back operator-evidence\BE-7\0047-VERIFY-RUN-V2.txt.
#
...
$Transcript    = Join-Path $EvidenceDir "0047-VERIFY-RUN-V2.txt"
...
if (!(Test-Path $ApplyRecPath)) {
    throw ("STOP: apply-final-state record missing: " + $ApplyRecPath + " . The verify act cannot precede the apply act. Run ITRGA_V2_0047_APPLY_PACK_V2.ps1 first.")
}
...
Write-Section "0047 VERIFY PACK V2 - ITRGA-V2-0047-VERIFY-PACK-V2"
...
if ($ApplyState["STATE_FILE_ID"] -ne "ITRGA-V2-0047-APPLY-FINAL-STATE-V2") { throw "STOP: apply state identity mismatch." }
...
$OutVer = & $Py -c "import alembic; print(alembic.__version__)" 2>&1
...
$VLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-VERIFY-FINAL-STATE-V2",
    ...
    "TRANSCRIPT=0047-VERIFY-RUN-V2.txt",
...
Write-Evidence ""
Write-Evidence "VERIFY PACK COMPLETE - PASS. The 0047 working-DB application act is proven end-to-end on the operator environment. The application may be restarted. Send back operator-evidence\BE-7\0047-APPLY-RUN-V2.txt and 0047-VERIFY-RUN-V2.txt to ITRGA."
