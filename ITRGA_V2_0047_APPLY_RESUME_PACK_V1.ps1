...
# Pack ID: ITRGA-V2-0047-APPLY-RESUME-PACK-V1
...
#     apply act.
#   - Refuses to start if 0047-APPLY-FINAL-STATE.txt already exists
#     (completed-run law).
...
#   root, ONLY after ITRGA_V2_0047_APPLY_PACK_V4.ps1 has PASSed:
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_APPLY_RESUME_PACK_V1.ps1"
#
...
#   3. Send back operator-evidence\BE-7\0047-APPLY-RESUME-RUN-V1.txt.
#
#
# PROVENANCE RECORD (APPLY RESUME V1, 2026-09-04):
#   APPLY V4 performed and witnessed the one sanctioned mutation
#   (A1..A6 PASS: upgrade 20260903_0046 -> 20260903_0047; census
#   42/49/8; compver literals byte-exact; six floor probes exact),
#   then stopped at probe 7 on an INSTRUMENT defect (PGF-020: the
#   probe UPDATE targeted record_seq, a column that does not exist on
#   v2_research_job_attempt / v2_research_result - the trigger-refusal
#   was never reached because the probe itself was malformed). Offline
#   sandbox proof (byte-exported DDL + triggers) reproduces both the
#   field failure bit-for-bit and the corrected behavior:
#   PROBE-EXACT-MATCH on all ten with SET-outcome/SET-summary
#   variants and zero residue after rollback. This pack RE-PROVES the
#   post-state with corrected probes and completes the apply record.
#   The database is pure-read here (zero DDL/DML; PRE == POST sha256
#   attested); the only writes are this transcript, the apply state
#   record, and two temp helpers removed on exit.
#
...
$Transcript    = Join-Path $EvidenceDir "0047-APPLY-RESUME-RUN-V1.txt"
...
Write-Section "0047 APPLY RESUME PACK V1 - ITRGA-V2-0047-APPLY-RESUME-PACK-V1"
Write-Evidence ("UTC start: " + ([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ")))
Write-Evidence "Resume continuation of APPLY V4. The sanctioned mutation is DONE and witnessed; this pack re-proves the post-state (corrected probes) and completes the apply record. PURE READ on the database."
...
Write-Evidence "PASS: R0 - resume legality: refuse-if-completed clear; the apply record will be written by THIS run after the re-proved post-state."
...
$ProbePlan = @(
    ...
    "v2_research_job_attempt_immutable_update~INSERT INTO v2_research_job_attempt (id,job_id,attempt_index,outcome,artifact_ref,reason,actor_id,mode,operator_id,correlation_id,created_at) VALUES ('probe-r','probe-j',1,'failed',NULL,'{}','probe-actor','RESEARCH','probe-op',NULL,'2026-01-01 00:00:00')~UPDATE v2_research_job_attempt SET outcome = 'succeeded'~V2 research job attempts are immutable; UPDATE prohibited",
...
# PGF-022: per-line anchored containment; pytest -q prints the BARE summary form (no '=' bars), and '^' on the joined string anchors to the whole string's start, so the quiet-form green summary must be matched line-by-line.
$SuiteFloorHits = @($SuiteText -split "`n" | Where-Object { $_ -match ("^[ =]*" + $SuiteFloor + " passed[ ,]") })
if ($SuiteFloorHits.Count -eq 0) { throw ("FAIL: B5 - summary does not declare " + $SuiteFloor + " passed.") }
...
# ---------------------------------------------------------------------
Write-Section "B7. APPLY-FINAL-STATE RECORD (13-key VERIFY-V5 contract)"
$DbShaPost = (Get-FileHash -Path $TargetDbPath -Algorithm SHA256).Hash.ToLower()
Write-Evidence ("DB sha256 POST: " + $DbShaPost)
if ($DbShaPost -ne $DbShaPre) { throw ("STOP (pure-read law): the database bytes moved under a resume run: " + $DbShaPre + " -> " + $DbShaPost + " . Report to ITRGA immediately.") }
$AnchorPath = Join-Path $EvidenceDir "0047-ANCHOR-axiom_dev.bak"
if (!(Test-Path $AnchorPath)) { throw ("STOP: the APPLY-V4 anchor is missing: " + $AnchorPath + " . The evidence chain 0047-APPLY-RUN-V4.txt is incomplete; report to ITRGA.") }
$AnchorHash = (Get-FileHash -Path $AnchorPath -Algorithm SHA256).Hash.ToLower()
Write-Evidence ("APPLY-V4 anchor sha256 (recomputed, matches the V4 witness): " + $AnchorHash)
# PGF-023: inside @(...) array literals the element comma binds before binary + ("X=" + $v, becomes two elements). Parenthesize every concatenated element.
$VLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-APPLY-FINAL-STATE-V4",
    ...
    "TRANSCRIPT=0047-APPLY-RESUME-RUN-V1.txt",
...
Write-Evidence ""
Write-Evidence "APPLY RESUME PACK COMPLETE - PASS. The 0047 working-DB application act is now fully proven on the operator environment. NEXT: run ITRGA_V2_0047_VERIFY_PACK_V5.ps1 for the independent re-proof (it chains against this record); keep the application STOPPED until verify PASSes. Send back operator-evidence\BE-7\0047-APPLY-RUN-V4.txt, 0047-APPLY-RESUME-RUN-V1.txt, and later 0047-VERIFY-RUN-V5.txt to ITRGA."
