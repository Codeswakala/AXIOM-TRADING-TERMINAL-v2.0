...
# ITRGA SQLITE VERIFY ACT EVIDENCE PACK (V2 - EVIDENCE-CONTINUITY RESUMPTION)
# Pack ID: ITRGA-V2-0046-VERIFY-PACK-V2
# Authority:
...
#     disclosure)
#   - ITRGA-V2-0046-APPLY-PACK-V1 (COMPLETED with verdict PASS on
#     2026-09-03: run started 18:44:46 +03:00, terminal write 18:45:24
#     +03:00; the PASS transcript is preserved by the Operator. The
#     PASS state record and the on-disk PASS transcript were then
#     destroyed by the duplicate-apply startup hygiene of the aborted
#     second apply run at 21:31:31 +03:00 - instrument event
#     E-0046-DUP. This V2 binds the apply act's identity to
#     ITRGA-embedded witnessed literals; the record's absence on disk
#     is required and asserted in B0e as the continuity marker. The
#     apply act CANNOT be re-run: its replay protection refuses on the
#     already-0046 baseline.)
...
#     (identity from ITRGA-embedded literals witnessed in the preserved
#     PASS transcript; abort unless the bytes match exactly).
...
#   - Re-verifies the recovery anchor (file hash equal to the embedded
#     witnessed anchor pins; the anchor is the CURRENT-AS-IS pre-image
#     per pre-act disclosure N-1: it lawfully includes the restart-era
#     bootstrap-admin row).
#
# PREREQUISITES:
#   - The apply act COMPLETED with verdict PASS on 2026-09-03
#     (witnessed by the Operator-preserved run transcript); the PASS
#     state record is ABSENT on disk (destroyed by the duplicate-apply
#     startup hygiene at 21:31:31 +03:00 - event E-0046-DUP; its
#     absence is the required continuity marker asserted in B0e).
#   - The application has NOT been started since the apply PASS (the
#     byte-binding enforces this; the
#     both-BE-6-tables-empty proof additionally assumes it).
#
# OPERATOR INSTRUCTION CARD:
#   1. The apply act is COMPLETE and replay-protected. Do NOT run the
#      apply pack again - its duplicate-run startup hygiene erases
#      evidence (event E-0046-DUP is the recorded example).
#   2. Execute ONLY this file, from the repository root:
#        powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0046_VERIFY_PACK_V2.ps1"
...
#        operator-evidence\BE-6\0046-VERIFY-RUN-V2.txt
#      (plus, for the E-0046-DUP record: the on-disk
#      operator-evidence\BE-6\0046-APPLY-RUN-V1.txt - the 21:31 abort -
#      and the Operator-preserved 18:44 PASS transcript paste).
#      This closes the 0046 application act and places the
...
$Transcript    = Join-Path $EvidenceDir "0046-VERIFY-RUN-V2.txt"
...
Write-Evidence "Pack: ITRGA-V2-0046-VERIFY-PACK-V2"
Write-Evidence "Act: band BE-6 (0046) working-database verification act (read-only; transactional probes roll back; V2 evidence-continuity resumption)"
...
Write-Evidence "State record: ABSENT on disk (E-0046-DUP continuity marker; required); identity bound to ITRGA-embedded witnessed literals in B0e."
...
try {
    ...
    Write-Section "B0e. APPLY-FINAL-STATE IDENTITY (E-0046-DUP continuity: record absence required; ITRGA-embedded witnessed literals)"

    # Instrument event E-0046-DUP: the 0046 apply act COMPLETED with
    # verdict PASS on 2026-09-03 (started 18:44:46 +03:00; terminal
    # post-write 18:45:24 +03:00); the preserved run transcript is the
    # witness. A duplicate apply run at 21:31:31 +03:00 (replay-protected
    # abort at gate A1 on the already-0046 baseline) deleted the PASS
    # state record and the PASS transcript at its startup hygiene step
    # before refusing. The anchor file and the abort transcript survive
    # on disk; the PASS transcript survives as the Operator-preserved
    # paste. The ITRGA certifies the following literals from that
    # witness; the record file must be ABSENT (its presence now would
    # signal a foreign write).

    if (Test-Path $StateRecPath) {
        throw "STOP: the 0046 apply-final-state record is present at ${StateRecPath} - but the E-0046-DUP continuity ruling requires it absent (the original was destroyed by the duplicate-run hygiene and no recreation is authorized). Report to ITRGA; run nothing."
    }

    $State = @{
        "STATE_FILE_ID"             = "ITRGA-V2-0046-APPLY-FINAL-STATE-V1"
        "RUN_TIMESTAMP"             = "2026-09-03 18:44:46 +03:00 (apply run start, witnessed; PASS record write completed after the 18:45:24 terminal write; exact stamp destroyed with the record - E-0046-DUP)"
        "POST_SIZE_BYTES"           = "1490944"
        "POST_LAST_WRITE"           = "2026-09-03 18:45:24 +03:00"
        "POST_SHA256"               = "5bd60aff6ab2d7fb6889b60363653b1d5e95d4405f15c0a7220a649ed233e795"
        "POST_REVISION"             = "20260903_0046"
        "POST_V2_TRIGGER_COUNT"     = "32"
        "POST_COMPVER_COUNT"        = "6"
        "POST_PERMISSION_COUNT"     = "41"
        "BASELINE_TIER1_BYTE_EXACT" = "False"
        "BASELINE_POLICY"           = "anchor-as-is (pre-act disclosure N-1)"
        "ANCHOR_FILENAME"           = "axiom_dev.db.pre-0046-20260903184515.bak"
        "ANCHOR_SIZE_BYTES"         = "1445888"
        "ANCHOR_SHA256"             = "7ac43e5f4495f17ad6057034075b14009d6b4d9d1d50e03347307e288dc7fce2"
    }

    Write-Evidence "Witnessed identity (ITRGA-embedded; identical key-set to the V1-consumed record):"
    ...
    Write-Evidence "PASS: B0e - record absence asserted as the E-0046-DUP continuity marker; verdict-binding post-0046 identity bound to ITRGA-embedded witnessed literals."
    ...
    Write-Section "B1. TARGET IDENTITY vs THE WITNESSED PASS IDENTITY (byte-exact binding)"

    $V1File = Get-Item ${TargetDbPath}
    $V1Size = $V1File.Length
    $V1Hash = (Get-FileHash ${TargetDbPath} -Algorithm SHA256).Hash.ToLower()
    Write-Evidence "  size       ${V1Size} bytes (witnessed $($State["POST_SIZE_BYTES"]))"
    Write-Evidence "  sha256     ${V1Hash} (witnessed $($State["POST_SHA256"]))"
    if ([string]$V1Size -ne $State["POST_SIZE_BYTES"]) {
        throw "FAIL: B1 - target file size differs from the witnessed 0046 PASS identity. The application may have been started since the 18:45:24 PASS write (or the file changed). Report to ITRGA."
    }
    if ($V1Hash -ne $State["POST_SHA256"]) {
        throw "FAIL: B1 - target file sha256 differs from the witnessed 0046 PASS identity. The application may have been started since the 18:45:24 PASS write (or the file changed). Report to ITRGA."
    }
    Write-Evidence "PASS: B1 - target file is byte-identical to the witnessed PASS identity (the state under verification is exactly the state the completed apply act produced on 2026-09-03)."
    ...
    Write-Evidence "  size       $($AnchorItem.Length) bytes (witnessed $($State["ANCHOR_SIZE_BYTES"]))"
    Write-Evidence "  sha256     ${AnchorHashNow} (witnessed $($State["ANCHOR_SHA256"]))"
    ...
    Write-Evidence ""
    Write-Evidence "VERIFY VERDICT: PASS - the working database in '${TargetDbPath}' is byte-identical to the witnessed PASS identity of the completed 0046 apply act (2026-09-03, run start 18:44:46 +03:00; PASS transcript preserved by the Operator; the state record was destroyed by duplicate-run startup hygiene - event E-0046-DUP - and its absence is asserted in B0e as the continuity marker) and independently re-proves the BO-V2-BE-6-001 terminal state on the working lineage: revision exactly 20260903_0046 (the preserved transcript attests the single sanctioned upgrade); 32 v2 triggers exact-named (28 inherited intact + 4 BE-6 guards exact-messaged); both BE-6 tables with the pinned column name-sets, PK 'id', all seven named constraints in the stored DDL, both uniqueness anchors PROVEN BEHAVIORALLY and both indexes exact-named; 6 computation-version rows (BE-4 pinned by value; mge/sge and portfolio_risk_engine runtime-recomputed, the BE-6 recomputation equal to the pinned anchor); 41 permission rows (6 additive BE-6 exact, no duplicates); both BE-6 tables empty and all five BE-5 tables still empty; inherited BE-1..BE-3 anchors content-exact; drift exactly the 9 inherited V1 tokens with zero BE-6/V2 tokens; no authority variable present; recovery anchor (current-as-is, pre-act disclosure N-1) verified byte-exact against the witnessed anchor pins."
    Write-Evidence "Transcript: ${Transcript}"
    Write-Evidence ""
    Write-Evidence "EVIDENCE ENVELOPE TO SEND TO ITRGA (closes the 0046 application act):"
    Write-Evidence "  operator-evidence\BE-6\0046-VERIFY-RUN-V2.txt"
    Write-Evidence "  operator-evidence\BE-6\0046-APPLY-RUN-V1.txt   (the 21:31 abort transcript - kept for the E-0046-DUP record)"
    Write-Evidence "  the Operator-preserved 18:44 PASS transcript paste (the witness of the completed apply act)"
