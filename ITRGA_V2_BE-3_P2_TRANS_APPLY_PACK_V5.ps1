# =====================================================================
# AXIOM V2 BE-3 P2 STATUS TRANSITION - ITRGA SQLITE APPLY ACT EVIDENCE PACK (V5)
# Pack ID: ITRGA-V2-BE-3-P2-TRANS-APPLY-PACK-V5
# Authority:
...
# Pattern: ITRGA-V2-BE-3-P2-TRANS-APPLY-PACK-V3 (defect-corrected
#   structure; PGF-001..009 lessons incorporated by construction).
#   V3 delta vs V2 (minimal): input quote normalization at 0B (PGF-008).
#   V4 delta vs V3 (minimal): PGF-009 - all COMPOUND braced expansions
#   (${Expr.With.Calls}) removed (8 occurrences); PS 5.1 treats the
#   content of ${...} containing calls as a LITERAL variable name.
#   Replaced by bare expressions (statement context) and $(...)
#   subexpressions (string context).
#   V5 delta vs V4 (single line): PGF-010 - "SELECT version()" (a
#   PostgreSQL function) corrected to "SELECT sqlite_version()"
#   (SQLite). Found by the ITRGA full dry run of V4 (B1 abort),
#   before any Operator execution. Everything else byte-identical to
#   V4. V5 verified by full dry run to a PASS verdict (dry-run record
#   in the ITRGA workspace).
#
# WHAT THIS PACK DOES:
...
#   C:\Users\victo\.vscode\AXIOM\axiom\ITRGA_V2_BE-3_P2_TRANS_APPLY_PACK_V5.ps1
#
# Run from the AXIOM repository root, as a file (exactly one command):
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_BE-3_P2_TRANS_APPLY_PACK_V5.ps1"
#
# Output (submit this one file to ITRGA):
#   C:\Users\victo\.vscode\AXIOM\axiom\operator-evidence\BE-3-P2-transition\BE-3-P2-TRANS-APPLY-EVIDENCE-V5.txt
# =====================================================================
...
$Transcript   = Join-Path $EvidenceDir "BE-3-P2-TRANS-APPLY-EVIDENCE-V5.txt"
...
Write-Evidence "Pack: ITRGA-V2-BE-3-P2-TRANS-APPLY-PACK-V5"
Write-Evidence "Pack note: V5 - PGF-010 (sqlite_version); PGF-009 (no compound braced expansions) and PGF-008 (input quote normalization) retained; all other logic identical"
