...
# Pack ID: ITRGA-V2-0047-APPLY-PACK-V1
# Authority:
...
#      (ITRGA-ISS-V2-0047-PACKS-001):
#        ITRGA_V2_0047_APPLY_PACK_V1.ps1   (this file; runs FIRST)
#        ITRGA_V2_0047_VERIFY_PACK_V1.ps1  (runs SECOND, after this PASS)
...
#        operator-evidence\BE-7\0047-APPLY-RUN-V1.txt
...
$Transcript    = Join-Path $EvidenceDir "0047-APPLY-RUN-V1.txt"
...
Write-Section "0047 APPLY PACK V1 - ITRGA-V2-0047-APPLY-PACK-V1"
...
if (Test-Path $NewMigPath) { throw ("STOP (startup hygiene): 0047 migration file already exists: " + $NewMigPath + " . A prior partial landing may exist; STOP and report to ITRGA.") }
...
foreach ($Rec in $FileRecords) {
    ...
    if ((Test-Path $Dest) -and (-not $IsModified)) {
        throw ("STOP (startup hygiene): new-file target already exists: " + $Dest + " . A prior partial landing may exist; STOP and report to ITRGA.")
    }
    if ($IsModified) {
...
$StateLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-APPLY-FINAL-STATE-V1",
    ...
    "TRANSCRIPT=0047-APPLY-RUN-V1.txt",
...
Write-Evidence ""
Write-Evidence "APPLY PACK COMPLETE - PASS. Chain at 20260903_0047 (head). Keep the application STOPPED. Next act: execute ITRGA_V2_0047_VERIFY_PACK_V1.ps1, then send back operator-evidence\BE-7\0047-APPLY-RUN-V1.txt and 0047-VERIFY-RUN-V1.txt. Restart the application only after the verify pack PASSes."
