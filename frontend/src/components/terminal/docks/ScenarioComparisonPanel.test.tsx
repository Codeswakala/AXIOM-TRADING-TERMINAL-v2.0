...
describe("ScenarioComparisonPanel", () => {
  it("renders two existing persisted scenarios side by side", () => {
    render(<ScenarioComparisonPanel reports={reports} onRefresh={vi.fn()} />);
    ...
    // UI-CONV-P03: the picker toggles selection — unchecking scenario-1 removes its card.
    const checkbox = screen.getByTestId("scenario-picker-checkbox-scenario-1");
    checkbox.click();
    expect(screen.queryByTestId("scenario-comparison-card-scenario-1")).not.toBeInTheDocument();
    expect(screen.getByTestId("scenario-comparison-card-scenario-2")).toBeInTheDocument();
  });
  ...
  it("requires at least two persisted scenarios for comparison", () => {
    render(<ScenarioComparisonPanel reports={[reports[0]]} />);
