# =====================================================================
# AXIOM V2 BE-3 P2 STATUS TRANSITION - ITRGA READ-ONLY VERIFY PACK (V2)
# Pack ID: ITRGA-V2-BE-3-P2-TRANS-VERIFY-PACK-V2
# Authority:
...
# Chain: sixth instrument of the BE-3 P2 transition chain (V2 of the
#   verify act; V1 superseded for re-runs on 2026-09-01, see V2 note). Predecessors: apply packs V1 (PG; retired for target),
...
#   C:\Users\victo\.vscode\AXIOM\axiom\ITRGA_V2_BE-3_P2_TRANS_VERIFY_PACK_V2.ps1
#
# Run from the AXIOM repository root, as a file (exactly one command):
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_BE-3_P2_TRANS_VERIFY_PACK_V2.ps1"
#
# Output (submit this one file to ITRGA):
#   C:\Users\victo\.vscode\AXIOM\axiom\operator-evidence\BE-3-P2-transition\BE-3-P2-TRANS-VERIFY-EVIDENCE-V2.txt
# =====================================================================
...
$Transcript   = Join-Path $EvidenceDir "BE-3-P2-TRANS-VERIFY-EVIDENCE-V2.txt"
...
$HelperDir  = Join-Path $env:TEMP "axiom_itrga_verify_v2"
...
try {
    ...
    Write-Section "BOUNDARY 9: alembic check (expected: inherited V1 drift set, plus the expected BE-4 schema set when the repository head is 20260831_0043)"
    ...
    # V2: the BE-4 schema drift expectation depends on the recorded
    # repository head (SECTION 3B). When 0043 is present in the
    # repository but NOT applied to the working database, the expected
    # drift set is the inherited 9 V1 tokens PLUS the three BE-4 tables
    # (the tables are absent from the working database). Any OTHER
    # v2_/ix_v2_ drift token is unexpected and fails the act.
    $Be4TableTokens = @(
    ...
    if ($RepoHead -eq "20260831_0043") {
        ...
        $Be4DriftNote = "PLUS the expected BE-4 schema set (the three BE-4 tables are absent from the working database: 0043 is present in the repository but NOT applied to it - no-touch preserved)"
    } elseif ($RepoHead -eq "20260829_0042") {
        ...
        $Be4DriftNote = "ONLY (no BE-4 schema drift: the repository head is 20260829_0042)"
    } else {
    ...
    $UnexpectedV2Tokens = @()
    foreach ($M in $DriftNameMatches) {
        $Name = $M.Groups[1].Value
        if (($Name -like "v2_*" -or $Name -like "ix_v2_*") -and ($KnownBe4Tokens -notcontains $Name)) {
            $UnexpectedV2Tokens += $Name
        }
    }
    ...
    Write-Evidence "PASS: B9 - alembic check shows the inherited V1 drift set (all 9 expected tokens present, no V2/P2/transition drift, non-zero exit as expected) ${Be4DriftNote}."
    ...
    if ($RepoHead -eq "20260831_0043") {
        $TerminalDriftText = "alembic current revision 20260829_0042 (repository head 20260831_0043 recorded in SECTION 3B), drift = inherited V1 set + expected BE-4 schema set (0043 in repository, not applied to the working database)"
    } else {
