...
# Pack ID: ITRGA-V2-0047-VERIFY-PACK-V4
...
#   on this run - the ADJUDICATION-final-state record is strict-parsed and
...
#   - Requires the adjudication-final-state record (strict KEY=VALUE parse;
...
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_VERIFY_PACK_V4.ps1"
#
...
#   3. Send back operator-evidence\BE-7\0047-VERIFY-RUN-V4.txt.
#
#

# PROVENANCE RECORD (0047 TERMINAL ACT, 2026-09-04):
#   Lineage: V1/V2 packs retired unused-stopped at PGF-017/PGF-018;
#   V3 apply proved the 0047 mutation was PRE-LANDED (floor gate saw
#   the chain already at 20260903_0047); V3 verify correctly refused
#   (order law - no apply-final-state exists by design under the
#   E-0046-DUP never-replay ruling). V4 therefore chains against the
#   adjudication-final-state record written by the 0047 ADJUDICATION
#   PACK V1 (pure-read attestation of the pre-landed state).
#   Everything else (B1..B7 floors) is the V3 verify body, repaired
#   for PGF-017 mid-line evidence concatenation and control-char
#   prompt hygiene.
#
# =====================================================================
...
$Transcript    = Join-Path $EvidenceDir "0047-VERIFY-RUN-V4.txt"
$ApplyRecPath  = Join-Path $EvidenceDir "0047-ADJUDICATE-FINAL-STATE.txt"
...
if (!(Test-Path $ApplyRecPath)) {
    throw ("STOP: adjudication-final-state record missing: " + $ApplyRecPath + " . The verify act cannot precede the adjudication act. Run ITRGA_V2_0047_ADJUDICATE_PACK_V1.ps1 first.")
}
...
Write-Section "0047 VERIFY PACK V4 - ITRGA-V2-0047-VERIFY-PACK-V4"
...
if (Test-Path $VerifyRecPath) { throw ("STOP (refuse-if-completed, E-0046-DUP law): a completed verify record already exists: " + $VerifyRecPath + " . Report to ITRGA before running anything further.") }
$ApplyState = Read-Pins -Path $ApplyRecPath -ExpectedKeys @(
    "STATE_FILE_ID","MODE","TARGET_REVISION","PRIOR_HEAD_OBSERVED","PRE_POST_DB_HASH_EQUAL",
    "TRIGGERS_TOTAL","PERMISSIONS_TOTAL","COMPVER_TOTAL","COMPVER_RPE_HASH","COMPVER_RJE_HASH",
    "SUITE_RESULT","DB_PATH","DB_SHA256_PRE","DB_SHA256_POST","TRANSCRIPT","UTC_COMPLETED") -Label "adjudicate-final-state"
if ($ApplyState["STATE_FILE_ID"] -ne "ITRGA-V2-0047-ADJUDICATE-FINAL-STATE-V1") { throw "STOP: adjudication state identity mismatch." }
if ($ApplyState["MODE"] -ne "ADJUDICATED-PRIOR-COMPLETION") { throw "STOP: adjudication state mode mismatch - the record must declare ADJUDICATED-PRIOR-COMPLETION." }
if ($ApplyState["PRIOR_HEAD_OBSERVED"] -ne "20260903_0047") { throw "STOP: adjudication record must declare the pre-landed head 20260903_0047." }
if ($ApplyState["PRE_POST_DB_HASH_EQUAL"] -ne "TRUE") { throw "STOP: adjudication record must declare PRE/POST hash equality (pure-read law)." }
if ($ApplyState["DB_SHA256_PRE"] -ne $ApplyState["DB_SHA256_POST"]) { throw "STOP: adjudication record PRE/POST hashes differ." }
if (!(Test-Path (Join-Path $EvidenceDir $ApplyState["TRANSCRIPT"]))) { throw "STOP: adjudication run transcript missing beside the record: " + $ApplyState["TRANSCRIPT"] }
if ($ApplyState["TARGET_REVISION"] -ne $ApplyTargetRevision) { throw "STOP: adjudication state revision mismatch." }
...
Write-Evidence "PASS: B0 - order law satisfied; adjudication record strict-parsed and cross-pinned against this pack's embedded literals."
...
$VLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-VERIFY-FINAL-STATE-V4",
    "TARGET_REVISION=" + $ApplyTargetRevision,
    "ADJUDICATE_STATE_RECORD=0047-ADJUDICATE-FINAL-STATE.txt",
    ...
    "TRANSCRIPT=0047-VERIFY-RUN-V4.txt",
...
Write-Evidence ""
Write-Evidence "VERIFY PACK COMPLETE - PASS. The 0047 working-DB application act is proven end-to-end on the operator environment. The application may be restarted. Send back operator-evidence\BE-7\0047-ADJUDICATE-RUN-V1.txt and 0047-VERIFY-RUN-V4.txt to ITRGA."
