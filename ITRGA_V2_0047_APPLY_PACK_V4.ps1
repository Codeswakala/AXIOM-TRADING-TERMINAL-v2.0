...
# Pack ID: ITRGA-V2-0047-APPLY-PACK-V4
# Authority:
...
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_APPLY_PACK_V4.ps1"
...
#      (ITRGA-ISS-V2-0047-PACKS-005):
#        ITRGA_V2_0047_APPLY_PACK_V4.ps1   (this file; runs FIRST)
#        ITRGA_V2_0047_VERIFY_PACK_V5.ps1  (runs SECOND, after this PASS)
...
#        operator-evidence\BE-7\0047-APPLY-RUN-V4.txt
#      (keep operator-evidence\BE-7\0047-APPLY-FINAL-STATE.txt in
#      place - the verify pack requires it.)
#
#
 PROVENANCE RECORD (APPLY V4, 2026-09-04):
#   Adjudication run (field-witnessed, pure-read) proved the true state:
#   the 17 delivered files ARE pre-landed and byte-exact (17/17 + six V1
#   pins), and the database is STILL at 20260903_0046 - the sanctioned
#   0047 mutation is GENUINELY PENDING. The earlier "pre-landed
#   migration" reading conflated alembic heads (script directory) with
#   alembic current (database row); recorded as PGF-019: never narrate
#   heads as DB state. V3 apply therefore refused at a gate whose
#   expectation assumed a pristine tree; V4 retunes exactly that
#   expectation (single script head == 20260903_0047, matching the
#   hash-adjudicated pre-landed migration file) and labels both values
#   explicitly. Everything else is the V3 apply body, unchanged.
...
$Transcript    = Join-Path $EvidenceDir "0047-APPLY-RUN-V4.txt"
...
Write-Section "0047 APPLY PACK V4 - ITRGA-V2-0047-APPLY-PACK-V4"
...
$OutVer = Invoke-Capped $Py @('-c', "import sys, alembic; print(sys.version.split()[0]); print(alembic.__version__)")
...
$HeadLinesAny = @(@($OutHeads) | Where-Object { ([string]$_) -match "^(rev|[0-9a-f]{8})" -and ([string]$_) -notmatch "^INFO" })
if ($HeadLinesAny.Count -ne 1) { throw ("STOP: floor gate - expected a single script head (PGF-019 law: heads reports the script directory); observed: '" + (Norm-Text $OutHeads) + "'.") }
if (([string]$HeadLinesAny[0]) -notmatch $ApplyTargetRevision) { throw ("STOP: floor gate - expected the single script head to be " + $ApplyTargetRevision + " naming the pre-landed, hash-adjudicated migration file; observed: '" + (Norm-Text $OutHeads) + "'.") }
Write-Evidence ("PASS: A1 floor - db current = " + $BaselineRevision + " (single; the mutation is pending); script head = " + $ApplyTargetRevision + " (single; pre-landed file to be hash-adjudicated at A4).")
...
$OutPre = Invoke-Capped $Py @($HelperCensus, $TargetDbPath)
...
$StateLines = @(
    ...
    "TRANSCRIPT=0047-APPLY-RUN-V4.txt",
...
Write-Evidence ""
Write-Evidence "APPLY PACK COMPLETE - PASS. Chain at 20260903_0047 (head). Keep the application STOPPED. Next act: execute ITRGA_V2_0047_VERIFY_PACK_V5.ps1, then send back operator-evidence\BE-7\0047-APPLY-RUN-V4.txt and 0047-VERIFY-RUN-V2.txt. Restart the application only after the verify pack PASSes."
