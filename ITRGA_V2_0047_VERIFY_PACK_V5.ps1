...
# Pack ID: ITRGA-V2-0047-VERIFY-PACK-V5
...
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_VERIFY_PACK_V5.ps1"
#
...
#   3. Send back operator-evidence\BE-7\0047-VERIFY-RUN-V5.txt.
#
#
# PROVENANCE RECORD (VERIFY V5, 2026-09-04):
#   The adjudication run proved: files pre-landed byte-exact; database
#   still at 20260903_0046. The terminal act is therefore APPLY V4 (the
#   one sanctioned mutation) followed by THIS verify, chained against
#   the APPLY-FINAL-STATE-V4 record. Body = verify V3 chassis with the
#   field-earned repairs: PGF-017 mid-line evidence concatenation, the
#   0x07 prompt control-char, clean venv candidate literals, and an
#   added refuse-if-completed check on this pack's own record.
#   PGF-019: alembic heads reports the script directory; alembic
#   current reports the database row.
...
$Transcript    = Join-Path $EvidenceDir "0047-VERIFY-RUN-V5.txt"
...
if (!(Test-Path $ApplyRecPath)) {
    throw ("STOP: apply-final-state record missing: " + $ApplyRecPath + " . The verify act cannot precede the apply act. Run ITRGA_V2_0047_APPLY_PACK_V4.ps1 first.")
}
...
Write-Section "0047 VERIFY PACK V5 - ITRGA-V2-0047-VERIFY-PACK-V5"
...
if ($ApplyState["STATE_FILE_ID"] -ne "ITRGA-V2-0047-APPLY-FINAL-STATE-V4") { throw "STOP: apply state identity mismatch." }
...
$VLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-VERIFY-FINAL-STATE-V5",
    ...
    "TRANSCRIPT=0047-VERIFY-RUN-V5.txt",
...
Write-Evidence ""
Write-Evidence "VERIFY PACK COMPLETE - PASS. The 0047 working-DB application act is proven end-to-end on the operator environment. The application may be restarted. Send back operator-evidence\BE-7\0047-APPLY-RUN-V4.txt and 0047-VERIFY-RUN-V5.txt to ITRGA."
