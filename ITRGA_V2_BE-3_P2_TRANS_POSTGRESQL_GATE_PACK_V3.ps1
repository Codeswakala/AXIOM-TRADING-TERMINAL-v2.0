# =====================================================================
# AXIOM V2 BE-3 P2 STATUS TRANSITION - ITRGA POSTGRESQL GATE EVIDENCE PACK (V3)
# Pack ID: ITRGA-V2-BE-3-P2-TRANS-PG-GATE-PACK-V3
# Supersedes: ITRGA-V2-BE-3-P2-TRANS-PG-GATE-PACK-V2 (its 2026-08-29 run is the
#   accepted gate evidence; V3 corrects only the boundary-10 expectation per
#   PGF-006) and V1 (withdrawn - parse defect PGF-005, never executed)
# Authority:
...
#
# Corrections vs V2 (ITRGA-owned pack defect, recorded 2026-08-29):
#   PGF-006: boundary 10 asserted the plan Part 9 / runbook step 10
#            expectation as written ("inherited V1 drift set ONLY"). That
#            expectation is UNATTAINABLE at this sequence's terminal
#            revision: the sequence ends one revision below head
#            (20260825_0041 vs head 20260829_0042) BY DESIGN - the reversal
#            proof (B7) is permanent on this database and the re-upgrade is
#            permanently refused by P-5 (B8). `alembic check` at a non-head
#            revision reports "Target database is not up to date." (non-zero
#            exit) WITHOUT computing the drift set. V2's run aborted on this
#            assertion alone; no database boundary deviated. The corrected
#            boundary 10 expects exactly that by-design outcome. The
#            no-V2-drift property is established separately (B10 notes in
#            the boundary sequence; ITRGA-DET-V2-BE-3-P2-TRANS-FINAL-001
#            section 5).
#
...
#   C:\Users\victo\.vscode\AXIOM\axiom\ITRGA_V2_BE-3_P2_TRANS_POSTGRESQL_GATE_PACK_V3.ps1
#
# Run from the AXIOM repository root, as a file (exactly one command):
#   powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_BE-3_P2_TRANS_POSTGRESQL_GATE_PACK_V3.ps1"
#
# Output (submit this one file to ITRGA):
#   C:\Users\victo\.vscode\AXIOM\axiom\operator-evidence\BE-3-P2-transition\BE-3-P2-TRANS-GATE-EVIDENCE-V3.txt
# =====================================================================
...
$Transcript   = Join-Path $EvidenceDir "BE-3-P2-TRANS-GATE-EVIDENCE-V3.txt"
...
Write-Evidence "Pack: ITRGA-V2-BE-3-P2-TRANS-PG-GATE-PACK-V3"
Write-Evidence "Supersedes: V2 (boundary-10 expectation corrected per PGF-006; V2's 2026-08-29 run is the accepted gate evidence) and V1 (withdrawn - PGF-005)"
...
try {
    ...
    Write-Section "BOUNDARY 10: alembic check (PGF-006 corrected expectation at the terminal revision)"

    Write-Evidence "Expected outcome at terminal revision 20260825_0041 (head 20260829_0042):"
    Write-Evidence "  non-zero exit + 'Target database is not up to date.' - the correct, by-design outcome:"
    Write-Evidence "  the sequence ends one revision below head because the reversal proof (B7) is"
    Write-Evidence "  permanent on this database and the re-upgrade is permanently refused by P-5 (B8)."
    Write-Evidence "  'alembic check' at a non-head revision reports this WITHOUT computing the drift set."
    Write-Evidence "  No-V2-drift property, established separately (ITRGA-DET-V2-BE-3-P2-TRANS-FINAL-001 section 5):"
    Write-Evidence "  (a) accepted P2 gate proved 'alembic check' at the then-head (0041) on an identically"
    Write-Evidence "      built fresh chain = inherited V1 drift set ONLY (exact 9-token set);"
    Write-Evidence "  (b) 0042 is a data-only migration (ITRGA full read, Level I) - no tables/columns/"
    Write-Evidence "      indexes added or removed, so the drift set at 0042 equals the drift set at 0041;"
    Write-Evidence "  (c) this run applied 0037 -> 0042 cleanly from a fresh baseline."
    ...
    if ($DriftCode -eq 0) {
        throw "FAIL: B10 - alembic check exited 0 at the terminal revision; expected non-zero."
    }
    if ($DriftText -notlike "*Target database is not up to date*") {
        throw "FAIL: B10 - unexpected alembic check output at the terminal revision (expected 'Target database is not up to date.')."
    }
    ...
    Write-Evidence "PASS: B10 - alembic check at the terminal revision reports the expected by-design outcome; no V2/P2/transition drift token present."
