...
#      paths pinned to the BE-6 Rev-2 literals).
...
$Transcript    = Join-Path $EvidenceDir "0046-APPLY-RUN-V1.txt"
...
# the BE-4/BE-5 acts; 0046 from the verified REM-Rev-2 literal.
...
$BaselineRevision    = "20260902_0045"
...
Write-Host ""
Write-Host "This pack applies band BE-6 (migration 20260903_0046) ONCE to the"
...
$HelperDir  = Join-Path $env:TEMP "axiom_itrga_0046_apply_v1"
...
Write-Evidence "Read-write scope: exactly one mutation (alembic upgrade 20260903_0046) on the target file; plus the transcript, the anchor copy, and the apply-final-state record in operator-evidence\BE-6; plus the throwaway helper (removed on exit)."
...
try {
    ...
    Write-Section "A1. BASELINE (pre-mutation; embedded-constant authority; Tier-2 content re-proof; abort on any failure)"
    ...
    $MainBackupPath = Join-Path $EvidenceDir (${LeafName} + ".pre-0046-" + ${BackupStamp} + ".bak")
    ...
    Write-Section "A3. PROVENANCE (36 files re-pinned on disk: 9 chain migrations + 27 delivered files; abort on any mismatch)"
    ...
    Write-Evidence "PASS: A3 - all 36 files on disk hash to the pinned values (9 chain migrations 0038-0046; 27 delivered BE-5/BE-6 files per REM-001 Rev 2 and REM-Rev-2; the four shared paths pinned to the BE-6 Rev-2 literals)."
    ...
    Write-Evidence "PASS: A5.1 (T-1) - current revision is exactly ${ApplyTargetRevision} (chain 0043 -> 0044 -> 0045 -> 0046)."
    ...
    Write-Evidence "PASS: A5.6 - both BE-6 tables are empty; all five BE-5 tables remain empty."
    ...
    Write-Section "A7. DRIFT RE-BASELINE: alembic check (expected: exactly the 9 inherited V1 tokens; zero BE-6/V2 tokens; format-independent)"
    ...
    if ($DriftText -match $DriftBanPattern) {
        throw "FAIL: A7 (T-10) - V2/BE-6 drift token detected in alembic check output; after the 0046 apply the BE-6 schema set (like BE-5 before it) must have left the drift set."
    }
    ...
    Write-Evidence "NEXT: run ITRGA_V2_0046_VERIFY_PACK_V1.ps1 NOW - BEFORE restarting the application (the verify pack requires the two new tables to still be empty and binds to this run's state record)."
