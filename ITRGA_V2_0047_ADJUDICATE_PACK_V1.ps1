...
# Pack ID: ITRGA-V2-0047-ADJUDICATE-PACK-V1
...
#   on this run - the state is adjudicated PURE-READ and the results are
...
#   - Refuses-if-completed on its own state record (E-0046-DUP law):
#     a completed adjudication is never replayed.
#   - Refuses to start if 0047-ADJUDICATE-FINAL-STATE.txt already exists
#     (completed-run law).
...
#   root, ONLY after ITRGA_V2_0047_ADJUDICATE_PACK_V1.ps1 has PASSed:
#     powershell -NoProfile -ExecutionPolicy Bypass -File ".\ITRGA_V2_0047_ADJUDICATE_PACK_V1.ps1"
#
...
#   3. Send back operator-evidence\BE-7\0047-ADJUDICATE-RUN-V1.txt.
#
#

# PROVENANCE RECORD (0047 TERMINAL ACT, 2026-09-04):
#   The sanctioned mutation (DB upgrade 20260903_0046 -> 20260903_0047
#   + 17 files) was discovered PRE-LANDED on the operator environment:
#   V1 pack refused at A1 (migration file already on the tree); V2 pair
#   retired at the PGF-017 evidence-printing law; V3 apply proved via
#   its floor gate that the chain is ALREADY at 20260903_0047 (head);
#   V3 verify correctly refused for missing apply-final-state (order
#   law). ZERO mutations occurred across all refusals. Ruling:
#   E-0046-DUP - never replay a completed upgrade. This pack therefore
#   forensically adjudicates the pre-landed state byte-for-byte and
#   attests the full post-state floor, with a pure-read guarantee
#   proven by DB sha256 PRE == POST, then writes the adjudication
#   state record that the verify act (V4) trusts-chain against.
#
# =====================================================================
...
$Transcript    = Join-Path $EvidenceDir "0047-ADJUDICATE-RUN-V1.txt"
$AdjRecPath = Join-Path $EvidenceDir "0047-ADJUDICATE-FINAL-STATE.txt"
...
if (Test-Path $AdjRecPath) {
    throw ("STOP (E-0046-DUP law): a completed verify record already exists: " + $AdjRecPath + " . Refusing to disturb it; report to ITRGA if you believe it is stale.")
}
...
Write-Section "0047 ADJUDICATION PACK V1 - ITRGA-V2-0047-ADJUDICATE-PACK-V1"
Write-Evidence ("UTC start: " + ([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ")))
Write-Evidence "Forensic adjudication + attestation of a PRE-LANDED 0047 state. PURE READ: zero mutations of the database and the tree; only this transcript, the state record, and two temp helper files (removed on exit) are written."
Write-Evidence "Precondition law: the alembic chain must already be at 20260903_0047 and the 17 delivered files already on the tree. Any divergence fails-close with an escalation report to ITRGA; nothing is altered."
...
if (Test-Path $AdjRecPath) { throw ("STOP (refuse-if-completed, E-0046-DUP law): a completed adjudication record already exists: " + $AdjRecPath + " . Never replay a completed act; report to ITRGA before running anything further.") }
Write-Evidence "PASS: A0 - startup hygiene, baseline pins cross-pinned, refuse-if-completed clear."
...
Write-Evidence ("DB sha256 PRE (pure-read provenance): " + $DbShaPre)

# ---------------------------------------------------------------------
# B2. FILE PINS (17 delivered + 6 V1)
# ---------------------------------------------------------------------
Write-Section "B2. DELIVERED-CORPUS BYTE ADJUDICATION (17 files, read-only)"
$FilePins = @(
...
foreach ($Fp in $FilePins) {
    ...
    if ($FpHash -ne $Parts[1]) { throw ("FAIL: B2 - delivered file hash drift: " + $Parts[0] + " = " + $FpHash + " != pinned " + $Parts[1] + " . The tree diverges from the accepted corpus versus the pre-landed state; report to ITRGA with this transcript.") }
...
Write-Evidence "PASS: B2 - ADJUDICATED: all 17 delivered files byte-identical to the accepted pins; six V1 pins intact. The pre-landed corpus is byte-exact."
...
# ---------------------------------------------------------------------
Write-Section "B7. ADJUDICATION-FINAL-STATE RECORD"
$DbShaPost = (Get-FileHash -Path $TargetDbPath -Algorithm SHA256).Hash.ToLower()
Write-Evidence ("DB sha256 POST: " + $DbShaPost)
if ($DbShaPost -ne $DbShaPre) { throw ("STOP (pure-read law): the database bytes moved under an adjudication run: " + $DbShaPre + " -> " + $DbShaPost + " . A probe or suite-side effect escaped the transactional law; report to ITRGA immediately.") }
$VLines = @(
    "STATE_FILE_ID=ITRGA-V2-0047-ADJUDICATE-FINAL-STATE-V1",
    "MODE=ADJUDICATED-PRIOR-COMPLETION",
    "TARGET_REVISION=" + $ApplyTargetRevision,
    "PRIOR_HEAD_OBSERVED=20260903_0047",
    "PRE_POST_DB_HASH_EQUAL=TRUE",
    ...
    "DB_SHA256_PRE=" + $DbShaPre,
    "DB_SHA256_POST=" + $DbShaPost,
    "TRANSCRIPT=0047-ADJUDICATE-RUN-V1.txt",
    "UTC_COMPLETED=" + ([DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ssZ"))
)
$VLines | Set-Content -Path $AdjRecPath -Encoding UTF8
foreach ($Vl in $VLines) { Write-Evidence ("STATE: " + $Vl) }
Write-Evidence ""
Write-Evidence "ADJUDICATION PACK COMPLETE - PASS. The pre-landed 0047 state is byte-exact against the accepted corpus and the full post-state floor holds (chain, censuses, compver literals, guards, suite, drift). NEXT: run ITRGA_V2_0047_VERIFY_PACK_V4.ps1 for the independent re-proof; do NOT restart the application until the verify pack PASSes. Send back operator-evidence\BE-7\0047-ADJUDICATE-RUN-V1.txt and 0047-VERIFY-RUN-V4.txt to ITRGA."
