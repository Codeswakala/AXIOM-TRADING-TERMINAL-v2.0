<#
===============================================================================
ITRGA_V2_BE12_LOCKREFUSAL_400PIN_V1.ps1
===============================================================================
Pack:      ITRGA-V2-BE12-LOCKREFUSAL-400PIN-V1  (the ladder's TRUE LAST field act)
...
try:
 ...
 o=c.get('/api/v1/openapi.json')
 paths=list(o.json()['paths'].keys())
 lx=sorted(p for p in paths if '/live-exec' in p)
 print('ARM-ROUTES|count=%d'%len(lx))
 for p in lx: print('ARM-ROUTE|'+p)
 login=[p for p in paths if p.endswith('/auth/login')]
 if len(login)!=1:
  print('PYERROR|login path not unique: %r'%(login,)); sys.exit(2)
 r=c.post(login[0],json={'username':'admin','password':pw})
 print('ARM-LOGIN|http=%d'%r.status_code)
 if r.status_code!=200:
  print('PYERROR|login failed body=%r'%(r.text[:160],)); sys.exit(3)
 tok=r.json().get('access_token') or r.json().get('token')
 if not tok:
  print('PYERROR|no token key in login body'); sys.exit(4)
 h={'Authorization':'Bearer '+tok}
 ev=[p for p in lx if p.endswith('/evaluate')]
 if len(ev)!=1:
  print('PYERROR|evaluate path not unique: %r'%(ev,)); sys.exit(5)
 ...
 r=c.post(ev[0],json=body,headers=h)
...
function Invoke-Surface { param([string]$Tag, [bool]$ForceLive)
  ...
  try { $raw = & $Py -c $CodeSurface $Tag 2>&1; $code = $LASTEXITCODE }
  finally {
    Pop-Location
...
function Invoke-Door {
  Push-Location $Backend
  try { $raw = & $Py -c $CodeDoor $Target $ArmId 2>&1; $code = $LASTEXITCODE }
...
function Assert-RepoState {
  ...
  $lxeNow = & $Py -c $CodeLxe18 $Backend 2>&1
  Assert-Gate ((ConvertTo-Text @($lxeNow)).TrimEnd() -eq $Lxe18Expect) '18-file lxe build == 93f436bc... (arms run against the exact fielded build)' $Lxe18Expect '(see above)'
}
...
Write-Host 'Pack: ITRGA-V2-BE12-LOCKREFUSAL-400PIN-V1'
Write-Host 'Act: witness L1/L2/L4/L6 refusing on the fielded lineage (+ order fixtures). NOTHING LANDS.'
Write-Host 'Implements: ITRGA-V2-BE12-CLOSEOUT-ADDENDUM-CARD-0001 (governing text)'
Write-Host 'Authority: BO-V2-BE12E-001 closeout condition (i) / ITRGA_VERDICT_V2_BE12_DR5_GATE_001 SS4 / DR-2 / DR-F1 AM-2 / credential law N-O18'
...
$sweep = @(Get-ChildItem env: | Where-Object { $_.Name -match '^AXIOM_(TD|BROKER_PRACTICE|V2_MODE|ADMIN_PW|DATABASE_URL)' })
...
Assert-Gate ($r1.Text -match 'ARM-ROUTES\|count=22\r?\n') 'W1: exactly 22 live-exec routes enumerated (standing SS6-elective, recorded en passant)' 'count=22' '(see above)'
...
Assert-Gate ($r2.Text -match 'ARM-ROUTES\|count=22\r?\n') 'W2: exactly 22 live-exec routes enumerated' 'count=22' '(see above)'
